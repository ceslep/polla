<script>
  ...
  function animateValue(
    /** @type {(v: number) => void} */ setter,
    /** @type {number} */ target,
    /** @type {number} */ duration = 900,
  ) {
  ...
  const stats = $derived.by(() => {
    ...
    const goalsByMinute = minuteBands.map((_, i) => ({
      band: i,
      goals: 0,
      ht: 0,
      ft2h: 0,
    ...
    for (const m of finished) {
      ...
      // Goles por fecha
      const dKey = m.date;
      const cur = goalsByDate.get(dKey) || {
        total: 0,
        ht: 0,
        ft2h: 0,
        count: 0,
      ...
      if (diff >= 4) {
        ...
        if (
          !biggestBlowout ||
          diff >
            Math.abs(biggestBlowout.score.ft[0] - biggestBlowout.score.ft[1])
        ) {
          biggestBlowout = m;
        }
      }
      ...
      if (ht) {
        const htLeader =
          ht[0] > ht[1] ? m.team1 : ht[1] > ht[0] ? m.team2 : null;
        const ftLeader =
          ft[0] > ft[1] ? m.team1 : ft[1] > ft[0] ? m.team2 : null;
        if (htLeader) {
          ...
          if (isFullComeback || isRescue) {
            const comebackTeam = isFullComeback
              ? ftLeader
              : htLeader === m.team1
                ? m.team2
                : m.team1;
            ...
            if (
              isFullComeback &&
              (!biggestComeback || htDeficit > biggestComeback._htDeficit)
            ) {
      ...
      if (t1HatTrick)
        hatTricks.push({
          match: m,
          team: /** @type {1} */ (1),
          goals: (m.goals1 || []).length,
        });
      if (t2HatTrick)
        hatTricks.push({
          match: m,
          team: /** @type {2} */ (2),
          goals: (m.goals2 || []).length,
        });
    ...
    // Minuto de oro
    const goldenMinute = goalsByMinute.reduce(
      (a, b) => (a.goals > b.goals ? a : b),
      goalsByMinute[0],
    );

    // Rachas
    const sortedMatches = finished
      .slice()
      .sort(
        (a, b) =>
    ...
    for (const [team, list] of teamMatches) {
      ...
      for (const item of list) {
        ...
        if (gf > 0) {
          scoreStreak++;
          maxScore = Math.max(maxScore, scoreStreak);
        } else scoreStreak = 0;
        if (gc === 0) {
          csStreak++;
          maxCs = Math.max(maxCs, csStreak);
        } else csStreak = 0;
      }
      if (maxScore > bestScoringStreak.streak)
        bestScoringStreak = { team, streak: maxScore };
      if (maxCs > bestCleanSheetStreak.streak)
        bestCleanSheetStreak = { team, streak: maxCs };
    }

    // ===== Análisis de remontadas =====
    // Ordenadas: primero las de mayor déficit remontado, luego más recientes.
    const comebackList = comebacks
      .slice()
      .sort(
        (a, b) => b.htDeficit - a.htDeficit || b.date.localeCompare(a.date),
      );
    ...
    for (const c of comebackList) {
      const e = comebackByTeam.get(c.team) || {
        team: c.team,
        wins: 0,
        draws: 0,
        total: 0,
      };
      if (c.type === "win") e.wins++;
      else e.draws++;
      e.total++;
    ...
    const excitingMatches = finished
      .map((m) => {
        ...
        if (score.ht) {
          const htLeader =
            score.ht[0] > score.ht[1]
              ? m.team1
              : score.ht[1] > score.ht[0]
                ? m.team2
                : null;
          const ftLeader =
            score.ft[0] > score.ft[1]
              ? m.team1
              : score.ft[1] > score.ft[0]
                ? m.team2
                : null;
      ...
      .filter(
        /** @type {(x: any) => x is {match: any, excitement: number}} */ (
          (x) => x !== null
        ),
      )
  ...
  const filterOptions = $derived.by(() => {
    ...
    for (const g of [...groups].sort()) {
      opts.push({
        value: /** @type {string} */ (g),
        label: String(g).replace("Group ", "Grupo "),
      });
    }
    return opts;
  });
  ...
  const maxDateGoals = $derived(
    stats.perDateRows.length
      ? Math.max(...stats.perDateRows.map((r) => r.total))
      : 1,
  );
  ...
  async function shareSummary() {
    ...
    const lines = [
      ...
      s.totalMatches
        ? `📊 ${s.finishedCount}/${s.totalMatches} partidos${phaseFilter !== "all" ? ` (${activeFilterLabel})` : ""}`
        : "",
      ...
      s.topScorer
        ? `👟 Goleador: ${s.topScorer.name} (${s.topScorer.goals})`
        : "",
      s.comebackStats.total
        ? `🔄 Remontadas: ${s.comebackStats.total} (${s.comebackStats.wins} vueltas)`
        : "",
      s.highestScoring
        ? `🔥 Más goles: ${nameOf(s.highestScoring.team1)} ${s.highestScoring.score.ft[0]}-${s.highestScoring.score.ft[1]} ${nameOf(s.highestScoring.team2)}`
        : "",
    ].filter(Boolean);
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          title: "Análisis de goles · Mundial 2026",
          text,
        });
        return;
      }
...
>
  ...
  <!-- svelte-ignore a11y_click_events_have_key_events -->
  <div
    class="bg-gray-900 text-white border border-white/10 rounded-none md:rounded-3xl w-full max-w-5xl h-full md:h-auto md:max-h-[92vh] overflow-hidden shadow-2xl flex flex-col animate-scale-in"
    onclick={(/** @type {MouseEvent} */ e) => e.stopPropagation()}
  >
    <!-- Header -->
    <div
      class="p-3 md:p-5 border-b border-white/10 flex justify-between items-center flex-shrink-0 gap-2"
    >
      <div class="flex items-center gap-2 md:gap-3 min-w-0">
        ...
        <div class="min-w-0">
          <h2
            id="goals-title"
            class="text-base md:text-xl font-bold text-cyan-400 truncate"
          >
            Análisis de goles
          </h2>
      ...
      <div class="flex items-center gap-1 shrink-0">
        {#if !isLoading && !error && stats.finishedCount > 0}
          <button
            class="h-9 md:h-10 px-2.5 md:px-3 shrink-0 flex items-center gap-1.5 text-xs md:text-sm font-semibold rounded-xl transition-colors {shareState ===
            'copied'
    ...
    <div class="border-b border-white/10 flex-shrink-0 overflow-x-auto">
      <div class="flex items-center gap-1 p-2 min-w-max">
        {#each [{ key: "resumen", label: "Resumen", emoji: "📊" }, { key: "partidos", label: "Por partido", emoji: "📋" }, { key: "equipos", label: "Por equipo", emoji: "🛡️" }, { key: "comparativas", label: "Comparativas", emoji: "⚔️" }, { key: "curiosidades", label: "Curiosidades", emoji: "✨" }] as t}
          <button
            class="px-3 py-2 md:px-4 md:py-2 rounded-lg text-xs md:text-sm font-semibold transition-all whitespace-nowrap {tab ===
            t.key
    ...
    <div class="flex-1 overflow-y-auto p-3 md:p-6">
      {#if !isLoading && !error && stats.finishedCount > 0 && tab !== "partidos" && tab !== "curiosidades" && filterOptions.length > 1}
        ...
        <div
          class="flex items-center gap-1.5 mb-3 md:mb-4 overflow-x-auto pb-1 -mx-1 px-1"
        >
          <span class="text-[10px] md:text-xs text-gray-500 shrink-0 mr-0.5"
            >Filtrar:</span
          >
          {#each filterOptions as opt (opt.value)}
            <button
              class="px-2.5 py-1 rounded-full text-[10px] md:text-xs font-semibold whitespace-nowrap transition-all {phaseFilter ===
              opt.value
                ? 'bg-cyan-600 text-white shadow shadow-cyan-900/30'
                : 'bg-white/5 text-gray-400 hover:text-white hover:bg-white/10'}"
              onclick={() => (phaseFilter = opt.value)}>{opt.label}</button
            >
          {/each}
        </div>
      {/if}
      ...
      {:else if error}
        <div class="text-center py-16">
          ...
          <button
            class="px-4 py-2 bg-cyan-600 hover:bg-cyan-500 rounded-xl text-sm font-bold"
            onclick={load}
          >
            🔄 Reintentar
          </button>
      ...
      {:else if tab === "resumen"}
        ...
        <div class="space-y-4 md:space-y-6">
          <!-- Cards de totales -->
          <div
            class="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3 stagger-children"
          >
            <div
              class="relative overflow-hidden bg-gradient-to-br from-emerald-500/15 to-cyan-500/10 border border-emerald-500/30 rounded-2xl p-3 md:p-4 animate-shine"
            >
              <div
                class="text-[10px] md:text-xs text-emerald-300/80 uppercase font-bold tracking-wider"
              >
                Goles totales
              </div>
              <div
                class="text-2xl md:text-4xl font-black text-emerald-400 tabular-nums animate-number-pop"
              >
                {dispTotalGoals}
              </div>
              <div class="text-[10px] md:text-xs text-gray-400 mt-1">
                {dispAvgGoals} / partido
              </div>
            </div>
            <div
              class="relative overflow-hidden bg-gradient-to-br from-amber-500/15 to-orange-500/10 border border-amber-500/30 rounded-2xl p-3 md:p-4 animate-shine"
              style="--i: 1"
            >
              <div
                class="text-[10px] md:text-xs text-amber-300/80 uppercase font-bold tracking-wider"
              >
                1.er tiempo
              </div>
              <div
                class="text-2xl md:text-4xl font-black text-amber-400 tabular-nums animate-number-pop"
              >
                {dispHtGoals}
              </div>
              <div class="text-[10px] md:text-xs text-gray-400 mt-1">
                {stats.totalGoals > 0
                  ? Math.round((stats.totalHtGoals / stats.totalGoals) * 100)
                  : 0}% del total
              </div>
            </div>
            <div
              class="relative overflow-hidden bg-gradient-to-br from-purple-500/15 to-pink-500/10 border border-purple-500/30 rounded-2xl p-3 md:p-4 animate-shine"
              style="--i: 2"
            >
              <div
                class="text-[10px] md:text-xs text-purple-300/80 uppercase font-bold tracking-wider"
              >
                2.º tiempo
              </div>
              <div
                class="text-2xl md:text-4xl font-black text-purple-400 tabular-nums animate-number-pop"
              >
                {disp2hGoals}
              </div>
              <div class="text-[10px] md:text-xs text-gray-400 mt-1">
                {stats.totalGoals > 0
                  ? Math.round((stats.total2hGoals / stats.totalGoals) * 100)
                  : 0}% del total
              </div>
            </div>
            <div
              class="relative overflow-hidden bg-gradient-to-br from-cyan-500/15 to-blue-500/10 border border-cyan-500/30 rounded-2xl p-3 md:p-4 animate-shine"
              style="--i: 3"
            >
              <div
                class="text-[10px] md:text-xs text-cyan-300/80 uppercase font-bold tracking-wider"
              >
                Partidos
              </div>
              <div
                class="text-2xl md:text-4xl font-black text-cyan-400 tabular-nums animate-number-pop"
              >
                {dispFinished}<span
                  class="text-sm md:text-lg text-gray-500 font-normal"
                  >/{stats.totalMatches}</span
                >
              </div>
              <div class="text-[10px] md:text-xs text-gray-400 mt-1">
          ...
          <div
            class="bg-white/5 border border-white/10 rounded-2xl p-3 md:p-5 animate-scale-in"
            style="animation-delay: 120ms"
          >
            <h3
              class="text-sm md:text-base font-bold text-white mb-3 md:mb-4 flex items-center gap-2"
            >
            ...
            {:else}
              <div
                class="flex items-end gap-1 md:gap-2 h-32 md:h-40 overflow-x-auto pb-1"
              >
                {#each stats.perDateRows as row, i (row.date)}
                  <div
                    class="flex-1 min-w-[40px] md:min-w-[60px] h-full flex flex-col items-center justify-end gap-1"
                    style="--i: {i}"
                  >
                    <div
                      class="text-[9px] md:text-xs text-gray-300 font-bold tabular-nums"
                    >
                      {row.total}
                    </div>
                    <div
                      ...
                      style="height: {(
                        (100 * row.total) /
                        maxDateGoals
                      ).toFixed(1)}%; animation-delay: calc(var(--i) * 60ms)"
                    ...
                    >
                      {#if row.ht > 0}
                        <div
                          ...
                          style="height: {((100 * row.ht) / row.total).toFixed(
                            1,
                          )}%"
                        ></div>
                      {/if}
                    ...
                    <div
                      class="text-[9px] md:text-[10px] text-gray-500 tabular-nums"
                    >
                      {shortDate(row.date)}
                    </div>
                  </div>
              ...
              <div
                class="flex items-center gap-3 mt-3 text-[10px] md:text-xs text-gray-400"
              >
                <span class="flex items-center gap-1"
                  ><span
                    class="w-2 h-2 md:w-3 md:h-3 rounded-sm bg-amber-500/70"
                  ></span>1.er tiempo</span
                >
                <span class="flex items-center gap-1"
                  ><span class="w-2 h-2 md:w-3 md:h-3 rounded-sm bg-emerald-400"
                  ></span>2.º tiempo</span
                >
              </div>
            {/if}
          </div>
          ...
          {#if stats.trendData.length > 1}
            {@const maxCumulative = Math.max(
              ...stats.trendData.map((d) => d.cumulative),
              1,
            )}
            {@const width = 1000}
            {@const height = 200}
            {@const padding = 24}
            {@const points = stats.trendData
              .map((d, i) => {
                const x =
                  padding +
                  (i / (stats.trendData.length - 1)) * (width - padding * 2);
                const y =
                  height -
                  padding -
                  (d.cumulative / maxCumulative) * (height - padding * 2);
                return `${x},${y}`;
              })
              .join(" ")}
            ...
            >
              <h3
                class="text-sm md:text-base font-bold text-white mb-2 md:mb-3 flex items-center gap-2"
              >
              ...
              <div class="relative h-40 md:h-52">
                ...
                >
                  ...
                  <path
                    ...
                    style="--line-length: {trendLineLength}px"
                  />
                  {#each stats.trendData as d, i}
                    {@const x =
                      padding +
                      (i / (stats.trendData.length - 1)) *
                        (width - padding * 2)}
                    {@const y =
                      height -
                      padding -
                    ...
                    <circle
                      ...
                      style="animation-delay: {0.8 + i * 0.06}s"
                    />
                  {/each}
                </svg>
              </div>
              <div class="flex justify-between text-[10px] text-gray-500 mt-1">
                ...
                <span
                  >{shortDate(
                    stats.trendData[stats.trendData.length - 1].date,
                  )}</span
                >
              </div>
            </div>
          ...
          >
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3">
              {#each [{ label: "Over 1.5", n: betting.over15, text: "text-emerald-400", bar: "bg-emerald-400" }, { label: "Over 2.5", n: betting.over25, text: "text-cyan-400", bar: "bg-cyan-400" }, { label: "Over 3.5", n: betting.over35, text: "text-purple-400", bar: "bg-purple-400" }, { label: "Ambos marcan", n: betting.bttsCount, text: "text-amber-400", bar: "bg-amber-400" }] as mk (mk.label)}
                {@const p = pct(mk.n, stats.finishedCount)}
                <div
                  class="bg-black/20 border border-white/10 rounded-xl p-2.5 md:p-3"
                >
                  <div class="text-[10px] md:text-xs text-gray-400">
                    {mk.label}
                  </div>
                  <div
                    class="text-lg md:text-2xl font-black tabular-nums {mk.text}"
                  >
                    {p}%
                  </div>
                  <div
                    class="h-1.5 bg-white/5 rounded-full overflow-hidden mt-1"
                  >
                    <div
                      class="h-full {mk.bar} animate-bar-grow"
                      style="width: {p}%; transform-origin: left; --line-length: 0"
                    ></div>
                  </div>
                  <div
                    class="text-[9px] md:text-[10px] text-gray-500 mt-1 tabular-nums"
                  >
                    {mk.n}/{stats.finishedCount}
                  </div>
                </div>
              {/each}
            </div>
            ...
            <div class="mt-3 md:mt-4">
              <div
                class="flex justify-between text-[10px] md:text-xs text-gray-400 mb-1"
              >
                <span
                  >🏠 Gana local {pct(
                    betting.homeWins,
              ...
              <div class="flex h-3 rounded-full overflow-hidden bg-white/5">
                <div
                  class="bg-emerald-500 animate-bar-grow"
                  style="width: {pct(
                    betting.homeWins,
                    stats.finishedCount,
                  )}%; transform-origin: left; --line-length: 0"
                  title="Local: {betting.homeWins}"
                ></div>
                <div
                  class="bg-gray-500 animate-bar-grow"
                  style="width: {pct(
                    betting.drawCount,
                    stats.finishedCount,
                  )}%; transform-origin: left; --line-length: 0"
                  title="Empate: {betting.drawCount}"
                ></div>
                <div
                  class="bg-cyan-500 animate-bar-grow"
                  style="width: {pct(
                    betting.awayWins,
                    stats.finishedCount,
                  )}%; transform-origin: left; --line-length: 0"
                  title="Visita: {betting.awayWins}"
                ></div>
              </div>
            </div>
            ...
            {#if betting.commonScores.length > 0}
              <div class="mt-3 md:mt-4">
                <div class="text-[10px] md:text-xs text-gray-400 mb-1.5">
                  Marcadores más comunes
                </div>
                <div class="flex flex-wrap gap-1.5">
                  {#each betting.commonScores as sc (sc.line)}
                    <span
                      class="px-2 py-1 rounded-lg bg-white/5 border border-white/10 text-[11px] md:text-xs font-mono text-cyan-300"
                    >
          ...
          {#if stats.phaseStats.groupMatches > 0 && stats.phaseStats.koMatches > 0}
            ...
            >
              ...
              <div class="space-y-3">
                {#each [{ label: "Fase de grupos", avg: ps.groupAvg, n: ps.groupMatches, text: "text-emerald-300", bar: "from-emerald-500 to-emerald-300" }, { label: "Eliminatoria", avg: ps.koAvg, n: ps.koMatches, text: "text-purple-300", bar: "from-purple-500 to-purple-300" }] as ph (ph.label)}
                  <div>
                    <div
                      class="flex justify-between text-[11px] md:text-xs mb-1"
                    >
                      <span class="font-semibold text-gray-200">{ph.label}</span
                      >
                      <span class="{ph.text} font-bold tabular-nums"
                        >{ph.avg} goles/partido · {ph.n} part.</span
                      >
                    </div>
                    <div
                      class="h-3 md:h-4 bg-white/5 rounded-full overflow-hidden"
                    >
                      <div
                        class="h-full bg-gradient-to-r {ph.bar} animate-bar-grow"
                        style="width: {((100 * ph.avg) / maxAvg).toFixed(
                          1,
                        )}%; transform-origin: left; --line-length: 0"
                      ></div>
                    </div>
          ...
          >
            <div
              class="bg-white/5 border border-white/10 rounded-2xl p-3 md:p-4"
              style="--i: 4"
            >
              <div class="text-[10px] md:text-xs text-gray-400">
                Porterías a cero
              ...
              <div
                class="text-xl md:text-2xl font-black text-white tabular-nums animate-number-pop"
              >
                {dispCleanSheets}
              </div>
            </div>
            ...
            <div
              class="bg-white/5 border border-white/10 rounded-2xl p-3 md:p-4 col-span-2 md:col-span-1"
              style="--i: 6"
            >
              ...
              <div
                class="text-xl md:text-2xl font-black text-white tabular-nums animate-number-pop"
              >
                {stats.hatTricks.length}
              </div>
            </div>
            <div
              class="bg-gradient-to-br from-pink-500/15 to-rose-500/10 border border-pink-500/30 rounded-2xl p-3 md:p-4 col-span-2 md:col-span-1"
              style="--i: 7"
            >
              <div
                class="text-[10px] md:text-xs text-pink-300/80 uppercase font-bold tracking-wider"
              >
                Minuto de oro
              </div>
              <div
                class="text-xl md:text-2xl font-black text-pink-400 tabular-nums"
              >
                {minuteBands[stats.goldenMinute.band]}'
              </div>
              <div class="text-[10px] md:text-xs text-gray-400 mt-1">
                {stats.goldenMinute.goals} goles
              </div>
            </div>
          </div>
          ...
          >
                ...
                >
                  ...
                  <div
                    ...
                    style="height: {(
                      (100 * band.goals) /
                      maxMinuteGoals
                    ).toFixed(1)}%; animation-delay: calc(var(--i) * 70ms)"
                    title="{minuteBands[band.band]}: {band.goals} goles"
                  >
                    {#if band.ht > 0}
                      <div
                        class="absolute bottom-0 left-0 right-0 bg-amber-500/70"
                        style="height: {((100 * band.ht) / band.goals).toFixed(
                          1,
                        )}%"
                      ></div>
                    {/if}
            ...
            >
              <span class="flex items-center gap-1"
                ><span class="w-2 h-2 md:w-3 md:h-3 rounded-sm bg-amber-500/70"
                ></span>1T</span
              >
              <span class="flex items-center gap-1"
                ><span class="w-2 h-2 md:w-3 md:h-3 rounded-sm bg-cyan-400"
                ></span>2T</span
              >
            </div>
          </div>
      ...
      {:else if tab === "partidos"}
        ...
        <div class="space-y-2 stagger-children">
          ...
          {#each stats.perMatch
            .slice()
            .sort((a, b) => b.date.localeCompare(a.date) || (b.time || "").localeCompare(a.time || "")) as m, i (i)}
            {@const expanded = expandedMatch === i}
            {@const is2hHeavy = m.ft2hGoals > m.htGoals}
            {@const isComeback =
              m.ht &&
              ((m.ht[0] < m.ht[1] && m.ft[0] > m.ft[1]) ||
                (m.ht[1] < m.ht[0] && m.ft[1] > m.ft[0]))}
            {@const isBlowout = Math.abs(m.ft[0] - m.ft[1]) >= 4}
            <div
              class="bg-white/5 border border-white/10 rounded-2xl overflow-hidden transition-all hover:border-cyan-500/30 hover:shadow-lg hover:shadow-cyan-900/10"
              style="--i: {i + 1}"
            >
              <button
              ...
              >
                <div class="flex items-center gap-2 md:gap-3">
                  <div
                    class="text-[10px] md:text-xs text-gray-500 w-12 md:w-16 shrink-0 tabular-nums"
                  >
                    {shortDate(m.date)}
                  </div>
                  <div class="flex-1 min-w-0">
                    <div
                      class="text-xs md:text-sm font-semibold truncate flex items-center gap-1 md:gap-1.5 flex-wrap"
                    >
                      {#if flagOf(m.team1)}<img
                          src={flagOf(m.team1)}
                          alt=""
                          class="h-3 w-4 md:h-4 md:w-5 object-cover rounded-sm shrink-0 ring-1 ring-white/10"
                        />{/if}
                    ...
                    <div
                      class="text-[10px] md:text-xs text-gray-500 mt-0.5 flex flex-wrap items-center gap-1"
                    >
                      {#if m.ht}
                        HT {m.ht[0]}-{m.ht[1]} ·
                        <span class="text-amber-400">1T {m.htGoals}</span> ·
                        <span class="text-emerald-400">2T {m.ft2hGoals}</span>
                        {#if is2hHeavy}<span class="text-purple-400"
                            >🔥 2.º T caliente</span
                          >{/if}
                      {:else}
                      ...
                      {#if m.group}
                        <span class="text-gray-600">
                          · Grupo {m.group.replace("Group ", "")}</span
                        >
                      {:else if m.round}
                        <span class="text-gray-600">
                          · {shortRound(m.round)}</span
                        >
                      {/if}
                      {#if isComeback}<span class="text-purple-400"
                          >🔄 Remontada</span
                        >{/if}
                      {#if isBlowout}<span class="text-red-400">💥 Goleada</span
                        >{/if}
                    </div>
                  </div>
                  <div class="text-gray-500 text-lg shrink-0">
                    {expanded ? "−" : "+"}
                  </div>
                </div>
              </button>
              {#if expanded && (m.goals1.length || m.goals2.length)}
                <div
                  class="px-3 md:px-4 pb-3 md:pb-4 pt-0 border-t border-white/5 animate-fade-in"
                >
                  <div
                    class="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-3 mt-2"
                  >
                    <div>
                      <div
                        class="text-[10px] md:text-xs text-gray-500 mb-1 flex items-center gap-1"
                      >
                        {#if flagOf(m.team1)}<img
                            src={flagOf(m.team1)}
                            alt=""
                            class="h-3 w-4 md:h-3.5 md:w-5 object-cover rounded-sm ring-1 ring-white/10"
                          />{/if}
                      ...
                      {:else}
                        <ul class="space-y-0.5 text-[11px] md:text-xs">
                          {#each m.goals1 as g}
                            <li
                              class="flex justify-between gap-2 text-gray-300"
                            >
                              <span class="truncate flex items-center gap-1">
                                <img
                                  src={flagOf(m.team1)}
                                  alt=""
                                  class="h-2.5 w-3.5 object-cover rounded-sm ring-1 ring-white/10 shrink-0"
                                  title={nameOf(m.team1)}
                                />
                                <span>{g.name}</span>
                              </span>
                              <span class="text-cyan-400 font-mono shrink-0"
                                >{g.minute}'</span
                              >
                            </li>
                          {/each}
      ...
      {:else if tab === "equipos"}
        ...
        <div class="space-y-4 md:space-y-6">
          ...
          >
            <h3
              class="text-sm md:text-base font-bold text-emerald-400 mb-3 flex items-center gap-2"
            >
            ...
            <div class="space-y-1.5 md:space-y-2 stagger-children">
              {#each stats.topAttacks as t, i (t.team)}
                ...
                >
                  ...
                  {/if}
                  <span class="w-20 md:w-32 truncate font-semibold"
                    >{nameOf(t.team)}</span
                  >
                  <div
                    class="flex-1 h-2 bg-white/5 rounded-full overflow-hidden"
                  >
                    <div
                      class="h-full bg-gradient-to-r from-emerald-500 to-emerald-300 animate-bar-grow"
                      style="width: {((100 * t.gfPerGame) / maxGf).toFixed(
                        1,
                      )}%; transform-origin: left; --line-length: 0"
                    ></div>
                  </div>
                  <span
                    class="w-14 md:w-20 text-right text-emerald-300 font-bold tabular-nums shrink-0"
                    >{t.gf} GF · {t.gfPerGame}/P</span
                  >
                </div>
              {/each}
            </div>
          ...
          >
            <h3
              class="text-sm md:text-base font-bold text-cyan-400 mb-3 flex items-center gap-2"
            >
            ...
            <div class="space-y-1.5 md:space-y-2 stagger-children">
              {#each stats.topDefenses as t, i (t.team)}
                ...
                >
                  ...
                  >
                    <div
                      class="h-full bg-gradient-to-r from-cyan-500 to-cyan-300 animate-bar-grow"
                      style="width: {((100 * t.gcPerGame) / maxGc).toFixed(
                        1,
                      )}%; transform-origin: left; --line-length: 0"
                    ></div>
                  </div>
                  <span
                    class="w-14 md:w-20 text-right text-cyan-300 font-bold tabular-nums shrink-0"
                    >{t.gc} GC · {t.gcPerGame}/P</span
                  >
                </div>
              {/each}
            </div>
          ...
          >
            <h3
              class="text-sm md:text-base font-bold text-purple-400 mb-1 flex items-center gap-2"
            >
              <span>🔥</span> Equipos del 2.º tiempo
            </h3>
            <p class="text-[10px] md:text-xs text-gray-500 mb-3">
              % de sus goles anotados en el segundo tiempo (mín. 3 goles)
            </p>
            <div class="space-y-1.5 md:space-y-2 stagger-children">
              {#each stats.secondHalfTeams as t, i (t.team)}
                ...
                >
                  ...
                  >
                    <div
                      class="h-full bg-gradient-to-r from-purple-500 to-pink-400 animate-bar-grow"
                      style="width: {t.secondHalfPct}%; transform-origin: left; --line-length: 0"
                    ></div>
                  </div>
                  <span
                    class="w-20 md:w-28 text-right text-purple-300 font-bold tabular-nums shrink-0"
                    >{t.gf2h}/{t.gf} = {t.secondHalfPct}%</span
                  >
                </div>
              {/each}
            </div>
          ...
          <!-- Rachas -->
          <div
            class="grid grid-cols-1 md:grid-cols-2 gap-3 animate-scale-in"
            style="animation-delay: 240ms"
          >
            <div
              class="bg-gradient-to-br from-amber-500/15 to-orange-500/10 border border-amber-500/30 rounded-2xl p-3 md:p-4"
            >
              <div
                class="text-[10px] md:text-xs text-amber-300/80 uppercase font-bold tracking-wider"
              >
              ...
              {#if stats.streaks.bestScoringStreak.streak > 1}
                <div
                  class="text-lg md:text-xl font-black text-white mt-1 flex items-center gap-2"
                >
                  {#if flagOf(stats.streaks.bestScoringStreak.team)}<img
                      src={flagOf(stats.streaks.bestScoringStreak.team)}
                      alt=""
                      class="h-4 w-6 rounded-sm ring-1 ring-white/10"
                    />{/if}
                  <span>{nameOf(stats.streaks.bestScoringStreak.team)}</span>
                </div>
                <div class="text-[11px] md:text-xs text-amber-400 mt-1">
                  {stats.streaks.bestScoringStreak.streak} partidos seguidos marcando
                </div>
              {:else}
                <div class="text-sm text-gray-500 mt-1">
                  Aún no hay racha destacada
                </div>
              {/if}
            </div>
            <div
              class="bg-gradient-to-br from-cyan-500/15 to-blue-500/10 border border-cyan-500/30 rounded-2xl p-3 md:p-4"
            >
              <div
                class="text-[10px] md:text-xs text-cyan-300/80 uppercase font-bold tracking-wider"
              >
                🧱 Racha sin recibir gol
              </div>
              {#if stats.streaks.bestCleanSheetStreak.streak > 1}
                <div
                  class="text-lg md:text-xl font-black text-white mt-1 flex items-center gap-2"
                >
                  {#if flagOf(stats.streaks.bestCleanSheetStreak.team)}<img
                      src={flagOf(stats.streaks.bestCleanSheetStreak.team)}
                      alt=""
                      class="h-4 w-6 rounded-sm ring-1 ring-white/10"
                    />{/if}
                  <span>{nameOf(stats.streaks.bestCleanSheetStreak.team)}</span>
                </div>
                <div class="text-[11px] md:text-xs text-cyan-400 mt-1">
                  {stats.streaks.bestCleanSheetStreak.streak} partidos seguidos imbatido
                </div>
              {:else}
                <div class="text-sm text-gray-500 mt-1">
                  Aún no hay racha destacada
      ...
      {:else if tab === "comparativas"}
        ...
        <div class="space-y-4 md:space-y-6">
          ...
          <div class="grid grid-cols-2 gap-2 md:gap-3">
            <div>
              <div
                class="text-[10px] md:text-xs text-cyan-400 font-bold uppercase tracking-wider mb-1 flex items-center gap-1.5"
              >
                {#if rowA && flagOf(rowA.team)}<img
                    src={flagOf(rowA.team)}
                    alt=""
                    class="h-3 w-4 rounded-sm ring-1 ring-white/10"
                  />{/if}
                Equipo A
              </div>
              <select
                class="w-full bg-gray-800 border border-cyan-500/30 hover:border-cyan-500/60 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500/30 rounded-xl px-2.5 py-2 text-xs md:text-sm text-white outline-none cursor-pointer transition-colors"
                style="color-scheme: dark"
                bind:value={teamA}
              >
                {#each stats.teamRows
                  .slice()
                  .sort( (a, b) => nameOf(a.team).localeCompare(nameOf(b.team)), ) as t (t.team)}
                  <option class="bg-gray-800 text-white" value={t.team}
                    >{nameOf(t.team)}</option
                  >
                {/each}
              </select>
            </div>
            <div>
              <div
                class="text-[10px] md:text-xs text-amber-400 font-bold uppercase tracking-wider mb-1 flex items-center gap-1.5"
              >
                {#if rowB && flagOf(rowB.team)}<img
                    src={flagOf(rowB.team)}
                    alt=""
                    class="h-3 w-4 rounded-sm ring-1 ring-white/10"
                  />{/if}
                Equipo B
              ...
              <select
                class="w-full bg-gray-800 border border-amber-500/30 hover:border-amber-500/60 focus:border-amber-500 focus:ring-1 focus:ring-amber-500/30 rounded-xl px-2.5 py-2 text-xs md:text-sm text-white outline-none cursor-pointer transition-colors"
                style="color-scheme: dark"
                bind:value={teamB}
              >
                {#each stats.teamRows
          ...
          {#if rowA && rowB && rowA.team !== rowB.team}
            <!-- Encabezado equipos -->
            <div
              class="flex items-center justify-between gap-2 bg-white/5 border border-white/10 rounded-2xl p-3 md:p-4 animate-scale-in"
            >
              <div class="flex items-center gap-2 min-w-0">
                {#if flagOf(rowA.team)}<img
                    src={flagOf(rowA.team)}
                    alt=""
                    class="h-6 w-9 md:h-8 md:w-12 object-cover rounded ring-1 ring-white/10 shrink-0"
                  />{/if}
                <div class="min-w-0">
                  <div
                    class="text-sm md:text-base font-black text-cyan-400 truncate"
                  >
                    {nameOf(rowA.team)}
                  </div>
                  <div class="text-[10px] md:text-xs text-gray-500">
                    {rowA.points} pts · {rowA.wins}V {rowA.draws}E {rowA.losses}D
                  </div>
                </div>
              </div>
              <span class="text-gray-600 font-black text-lg shrink-0">VS</span>
              <div
                class="flex items-center gap-2 min-w-0 justify-end text-right"
              >
                <div class="min-w-0">
                  <div
                    class="text-sm md:text-base font-black text-amber-400 truncate"
                  >
                    {nameOf(rowB.team)}
                  </div>
                  <div class="text-[10px] md:text-xs text-gray-500">
                    {rowB.points} pts · {rowB.wins}V {rowB.draws}E {rowB.losses}D
                  </div>
                </div>
                {#if flagOf(rowB.team)}<img
                    src={flagOf(rowB.team)}
                    alt=""
                    class="h-6 w-9 md:h-8 md:w-12 object-cover rounded ring-1 ring-white/10 shrink-0"
                  />{/if}
            ...
            >
              <div class="space-y-2.5 md:space-y-3">
                {#each [{ label: "Goles a favor", a: rowA.gf, b: rowB.gf, suffix: "" }, { label: "GF por partido", a: rowA.gfPerGame, b: rowB.gfPerGame, suffix: "" }, { label: "Goles en contra", a: rowA.gc, b: rowB.gc, suffix: "" }, { label: "GC por partido", a: rowA.gcPerGame, b: rowB.gcPerGame, suffix: "" }, { label: "Diferencia de gol", a: rowA.gd, b: rowB.gd, suffix: "" }, { label: "% goles 2.º tiempo", a: rowA.secondHalfPct, b: rowB.secondHalfPct, suffix: "%" }, { label: "% triunfos", a: rowA.winPct, b: rowB.winPct, suffix: "%" }, { label: "% ambos marcan", a: rowA.bttsPct, b: rowB.bttsPct, suffix: "%" }, { label: "Porterías a cero", a: rowA.cleanSheets, b: rowB.cleanSheets, suffix: "" }] as mt (mt.label)}
                  {@const span = Math.max(Math.abs(mt.a), Math.abs(mt.b), 0.01)}
                  <div>
                    <div
                      class="text-center text-[10px] md:text-xs text-gray-400 mb-0.5"
                    >
                      {mt.label}
                    </div>
                    <div class="flex items-center gap-2">
                      <span
                        class="w-10 md:w-14 text-right text-[11px] md:text-sm font-bold tabular-nums text-cyan-300 shrink-0"
                        >{mt.a}{mt.suffix}</span
                      >
                      <div class="flex-1 flex items-center gap-0.5">
                        <div
                          class="flex-1 h-2.5 md:h-3 bg-white/5 rounded-l-full overflow-hidden flex justify-end"
                        >
                          <div
                            class="h-full bg-gradient-to-l from-cyan-500 to-cyan-400 rounded-l-full animate-bar-grow"
                            style="width: {(
                              (100 * Math.abs(mt.a)) /
                              span
                            ).toFixed(
                              1,
                            )}%; transform-origin: right; --line-length: 0"
                          ></div>
                        </div>
                        <div
                          class="flex-1 h-2.5 md:h-3 bg-white/5 rounded-r-full overflow-hidden"
                        >
                          <div
                            class="h-full bg-gradient-to-r from-amber-500 to-amber-400 rounded-r-full animate-bar-grow"
                            style="width: {(
                              (100 * Math.abs(mt.b)) /
                              span
                            ).toFixed(
                      ...
                      <span
                        class="w-10 md:w-14 text-left text-[11px] md:text-sm font-bold tabular-nums text-amber-300 shrink-0"
                        >{mt.b}{mt.suffix}</span
                      >
                    </div>
                  </div>
            ...
            {#if radar}
              ...
              >
                <h3
                  class="text-sm md:text-base font-bold text-white mb-2 flex items-center gap-2"
                >
                ...
                <div class="flex flex-col items-center">
                  <svg viewBox="0 0 300 280" class="w-full max-w-sm h-auto">
                    ...
                    {#each radar.axes as ax, i (ax)}
                      ...
                      <text
                        ...
                        font-size="10"
                        text-anchor="middle"
                        dominant-baseline="middle">{ax}</text
                      >
                    {/each}
                    <!-- Polígono A -->
                    <polygon
                      points={radarPoints(radar.a, cx, cy, rad)}
                    ...
                    <!-- Polígono B -->
                    <polygon
                      points={radarPoints(radar.b, cx, cy, rad)}
                  ...
                  <div
                    class="flex items-center gap-4 text-[11px] md:text-xs mt-1"
                  >
                    <span class="flex items-center gap-1.5 text-cyan-300"
                      ><span class="w-3 h-3 rounded-sm bg-cyan-400"
                      ></span>{nameOf(rowA.team)}</span
                    >
                    <span class="flex items-center gap-1.5 text-amber-300"
                      ><span class="w-3 h-3 rounded-sm bg-amber-400"
                      ></span>{nameOf(rowB.team)}</span
                    >
                  </div>
                </div>
              </div>
          ...
          {:else if rowA && rowB && rowA.team === rowB.team}
            <div
              class="bg-white/5 border border-white/10 rounded-2xl p-6 text-center text-gray-400 text-sm animate-scale-in"
            >
      ...
      {:else if tab === "curiosidades"}
        ...
        <div class="space-y-3 md:space-y-4">
          ...
          {#if stats.topScorer}
            <div
              class="relative overflow-hidden bg-gradient-to-br from-amber-500/20 to-orange-500/10 border border-amber-500/40 rounded-2xl p-4 md:p-5 animate-scale-in animate-glow-pulse"
            >
              <div
                class="absolute inset-0 animate-shimmer opacity-30 pointer-events-none"
              ></div>
              <div class="relative flex items-center gap-3 md:gap-4">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div
                    class="text-lg md:text-2xl font-black text-white truncate"
                  >
                    {stats.topScorer.name}
                  </div>
                  <div
                    class="text-[11px] md:text-xs text-gray-300 flex items-center gap-2"
                  >
                    {#if flagOf(stats.topScorer.team)}<img
                        src={flagOf(stats.topScorer.team)}
                        alt=""
                        class="h-3 w-4 rounded-sm ring-1 ring-white/10"
                      />{/if}
                    <span>{nameOf(stats.topScorer.team)}</span>
                    <span class="text-amber-400 font-bold"
                      >{stats.topScorer.goals} goles</span
                    >
                  </div>
                </div>
              </div>
          ...
          {#if stats.highestScoring}
            ...
            <div
              class="bg-gradient-to-br from-amber-500/15 to-orange-500/10 border border-amber-500/30 rounded-2xl p-3 md:p-4 animate-scale-in"
            ...
            >
              ...
              <div
                class="text-sm md:text-base font-bold text-white mt-1 flex items-center gap-1.5 md:gap-2 flex-wrap"
              >
                {#if flagOf(m.team1)}<img
                    src={flagOf(m.team1)}
                    alt=""
                    class="h-4 w-6 md:h-5 md:w-7 object-cover rounded-sm ring-1 ring-white/10"
                  />{/if}
                <span>{nameOf(m.team1)}</span>
          ...
          {#if stats.biggestBlowout}
            ...
            <div
              class="bg-gradient-to-br from-red-500/15 to-pink-500/10 border border-red-500/30 rounded-2xl p-3 md:p-4 animate-scale-in"
              style="animation-delay: 120ms"
            >
              <div
                class="text-[10px] md:text-xs text-red-300/80 uppercase font-bold tracking-wider"
              >
                💥 Goleada más grande
              </div>
              <div
              ...
              <div class="text-[11px] md:text-xs text-gray-400 mt-1">
                {shortDate(m.date)} · {nameOf(winner)} ganó por {Math.abs(
          ...
          <!-- ===== Análisis de remontadas ===== -->
          <div
            class="bg-gradient-to-br from-purple-500/15 to-pink-500/10 border border-purple-500/30 rounded-2xl p-3 md:p-5 animate-scale-in"
            style="animation-delay: 180ms"
          >
            <div class="flex items-center justify-between gap-2 mb-3">
              <h3
                class="text-sm md:text-base font-bold text-purple-300 flex items-center gap-2"
              >
                <span>🔄</span> Remontadas
              </h3>
              <span class="text-[10px] md:text-xs text-gray-400"
                >basado en partidos con marcador al descanso</span
              >
            </div>
            ...
            {:else}
              <!-- Resumen -->
              <div
                class="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3 mb-3 md:mb-4"
              >
                <div
                  class="bg-black/20 border border-white/10 rounded-xl p-2.5 md:p-3 text-center"
                >
                  <div
                    class="text-xl md:text-3xl font-black text-purple-400 tabular-nums"
                  >
                    {cb.total}
                  </div>
                  <div
                    class="text-[9px] md:text-[10px] text-gray-400 uppercase tracking-wide"
                  >
                    Totales
                  </div>
                </div>
                ...
                >
                  <div
                    class="text-xl md:text-3xl font-black text-emerald-400 tabular-nums"
                  >
                    {cb.wins}
                  </div>
                  <div
                ...
                >
                  <div
                    class="text-xl md:text-3xl font-black text-pink-400 tabular-nums"
                  >
                    {cb.rate}%
                  </div>
                  <div
              ...
              {#if cb.epic}
                {@const e = cb.epic}
                <div
                  class="bg-gradient-to-r from-amber-500/15 to-purple-500/10 border border-amber-500/30 rounded-xl p-2.5 md:p-3 mb-3"
                >
                  <div
                    class="text-[10px] md:text-xs text-amber-300/90 uppercase font-bold tracking-wider mb-1"
                  >
                    🏆 Remontada épica · {e.htDeficit} goles de déficit
                  </div>
                  <div
                    class="text-xs md:text-sm font-bold text-white flex items-center gap-1.5 flex-wrap"
                  >
                    {#if flagOf(e.team)}<img
                        src={flagOf(e.team)}
                        alt=""
                        class="h-4 w-6 object-cover rounded-sm ring-1 ring-white/10"
                      />{/if}
                    <span>{nameOf(e.team)}</span>
                    <span class="text-gray-500">remontó a</span>
                    {#if flagOf(e.rival)}<img
                        src={flagOf(e.rival)}
                        alt=""
                        class="h-4 w-6 object-cover rounded-sm ring-1 ring-white/10"
                      />{/if}
                  ...
                  <div
                    class="text-[11px] md:text-xs text-gray-400 mt-1 font-mono"
                  >
                    HT {e.ht[0]}-{e.ht[1]} <span class="text-gray-600">→</span>
                    FT {e.ft[0]}-{e.ft[1]} · {shortDate(e.date)}
                  </div>
                </div>
              {/if}
                  ...
                  <div class="space-y-1.5 stagger-children">
                    {#each cb.kings as k, i (k.team)}
                      <div
                        ...
                        style="--i: {i}"
                      >
                        {#if flagOf(k.team)}<img
                            src={flagOf(k.team)}
                            alt=""
                            class="h-3 w-4 md:h-4 md:w-5 object-cover rounded-sm shrink-0 ring-1 ring-white/10"
                            title={nameOf(k.team)}
                          />{:else}<span class="w-4 md:w-5 shrink-0"
                          ></span>{/if}
                        ...
                        <span class="text-emerald-400 font-bold tabular-nums"
                          ...
                          <span class="text-gray-500 font-normal"
                            >vuelta{k.wins !== 1 ? "s" : ""}</span
                          ></span
                        >
                        {#if k.draws > 0}<span
                            class="text-amber-400 font-bold tabular-nums"
                            >{k.draws}
                            <span class="text-gray-500 font-normal"
                              >rescate{k.draws !== 1 ? "s" : ""}</span
                            ></span
                          >{/if}
                      </div>
                    {/each}
              ...
              <div class="text-[10px] md:text-xs text-gray-400 mb-1.5">
                Todas las remontadas ({cb.total})
              </div>
              <div class="space-y-1.5 stagger-children">
                {#each cb.list as c, i (c.date + c.team + c.rival)}
                  <div
                    class="flex items-center gap-2 bg-black/20 border border-white/5 rounded-lg px-2 py-1.5 text-[11px] md:text-xs"
                    style="--i: {i}"
                  >
                    <span
                      class="shrink-0"
                    ...
                    >
                    <div
                      class="flex-1 min-w-0 flex items-center gap-1 flex-wrap"
                    >
                      {#if flagOf(c.team)}<img
                          src={flagOf(c.team)}
                          alt=""
                          class="h-3 w-4 object-cover rounded-sm ring-1 ring-white/10 shrink-0"
                          title={nameOf(c.team)}
                        />{/if}
                      ...
                      {#if flagOf(c.rival)}<img
                          src={flagOf(c.rival)}
                          alt=""
                          class="h-3 w-4 object-cover rounded-sm ring-1 ring-white/10 shrink-0"
                          title={nameOf(c.rival)}
                        />{/if}
                    ...
                    <span class="font-mono text-gray-400 shrink-0">
                      <span class="text-amber-400"
                        >{c.isHomeTeam ? c.ht[0] : c.ht[1]}-{c.isHomeTeam
                          ? c.ht[1]
                          : c.ht[0]}</span
                      >
                      <span class="text-gray-600">→</span>
                      <span class="text-cyan-400"
                        >{c.isHomeTeam ? c.ft[0] : c.ft[1]}-{c.isHomeTeam
                          ? c.ft[1]
                          : c.ft[0]}</span
                      >
                    </span>
                    {#if c.htDeficit >= 2}<span
                        class="text-amber-400 shrink-0"
                        title="{c.htDeficit} goles de déficit">🔥</span
                      >{/if}
                    <span
                      class="text-gray-500 shrink-0 tabular-nums hidden sm:inline"
                      >{shortDate(c.date)}</span
                    >
                  </div>
                {/each}
              </div>
          ...
          {#if stats.excitingMatches.length > 0}
            <div
              class="bg-white/5 border border-white/10 rounded-2xl p-3 md:p-4 animate-scale-in"
              style="animation-delay: 240ms"
            >
              <h3
                class="text-sm md:text-base font-bold text-rose-400 mb-2 flex items-center gap-2"
              >
                    ...
                    >
                      ...
                      {#if m.score}
                        <span class="text-cyan-400 font-bold"
                          >{m.score.ft[0]}-{m.score.ft[1]}</span
                        >
                      {/if}
                      <span class="truncate">{nameOf(m.team2)}</span>
                      {#if flagOf(m.team2)}<img
          ...
          >
            <h3
              class="text-sm md:text-base font-bold text-cyan-400 mb-2 flex items-center gap-2"
            >
              <span>🎯</span> Hat-tricks ({stats.hatTricks.length})
            </h3>
            {#if stats.hatTricks.length === 0}
              <p class="text-[11px] md:text-xs text-gray-500">
                Aún no hay hat-tricks en el torneo.
              </p>
            {:else}
              <div class="space-y-1.5 stagger-children">
                {#each stats.hatTricks as ht, i (i)}
                  ...
                  <div
                    class="text-[11px] md:text-xs text-gray-300 flex flex-wrap items-center gap-1.5"
                    style="--i: {i}"
                  >
                    {#if flagOf(team)}<img
                        src={flagOf(team)}
                        alt=""
                        class="h-3 w-4 md:h-4 md:w-5 object-cover rounded-sm ring-1 ring-white/10"
                      />{/if}
          ...
          >
            <h3
              class="text-sm md:text-base font-bold text-amber-400 mb-2 flex items-center gap-2"
            >
              <span>👟</span> Top goleadores
            </h3>
            {#if stats.scorers.length === 0}
              <p class="text-[11px] md:text-xs text-gray-500">
                Sin goleadores registrados aún.
              </p>
            {:else}
                ...
                {#each stats.scorers.slice(0, 10) as s, i (s.name)}
                  ...
                  >
                    ...
                    {:else}
                      <span class="w-4 md:w-5 shrink-0"></span>
                    {/if}
                    <span class="flex-1 truncate text-gray-200 font-semibold"
                      >{s.name}</span
                    >
                    <span
                      class="text-[10px] text-gray-500 truncate hidden sm:inline"
                      >{s.team ? nameOf(s.team) : ""}</span
                    >
