...
Both match sources normalize to the `Match` shape (`types.js`). Note `api.js` has its **own** `TEAM_ALIASES_API`/`normalizeTeamNameApi` separate from `parser.js`'s — keep both in sync when adding teams. `compareBetWithMatch` is the scoring rule: exact score = **5 pts**, correct winner/direction = **3 pts**, else 0.
...
- Two divergent team-alias maps — `parser.js` (`TEAM_ALIASES`) vs `api.js` (`TEAM_ALIASES_API`); keep in sync when adding teams.
